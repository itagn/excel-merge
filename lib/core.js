...
async function mergeInit(resource, tag, type, num){
    try{
        ...
        if(type === 'simple'){
            ...
            for(let i=destObj.length;i<=resourceObj.length;i++){
                let random = (Math.random()+'').substring(2,8);
                destObj.push({name: 'Sheet'+ random, data: []})
            }
            let statBuffer = excel.build(destObj);
            fs.writeFileSync(destFile, statBuffer, { 'flag': 'w' });
            let buildArr = [];
...
function excelInit(arr=[], tag=[], type='simple', num=1){
    if(arr.length === 0 ){
        ...
        if(allow){
            if(num>=1){
                num = num | 0;
                if(type === 'simple' || type === 'mutiple'){
                    mergeInit(arr[0], tag, type, num-1);
                    arr.shift();
                    if(arr.length > 0){
                        excelInit(arr, tag, type, num);
                    }else{
